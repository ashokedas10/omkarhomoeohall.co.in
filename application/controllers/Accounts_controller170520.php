...
$id_header='',$id_detail='',$fromdate='',$todate='')
{
	...
	if($TranPageName=='Product_master')
	{
		...
		if($datatype=='SAVE')
		{
				...
				//HEADER SECTION				
				$save_hdr['hsncode']=mysql_real_escape_string($form_data->hsncode);
				$save_hdr['productname']=mysql_real_escape_string($form_data->productname);
				$save_hdr['group_id']=mysql_real_escape_string($form_data->group_id);
				$save_hdr['tax_ledger_id']=mysql_real_escape_string($form_data->tax_ledger_id);
				$save_hdr['brand_id']=mysql_real_escape_string($form_data->brand_id);
				$save_hdr['product_type']='FINISH';
				
				if(mysql_real_escape_string($form_data->product_id)>0)
				{						
					$id_header=mysql_real_escape_string($form_data->product_id);
	...
	if($TranPageName=='PurchaseEntry')
	{
		...
		if($datatype=='SAVE')
		{
			...
			$save_details['product_id']=mysql_real_escape_string($form_data->product_id);
			...
			$save_details['rackno']=mysql_real_escape_string($form_data->rackno);
			...
			if($save_details['product_id']>0 )
			{
					...
					{	
					//	if($param_array['TRANSACTION']<>'NO_ADD_EDIT')
						//{}
							$this->projectmodel->save_records_model('','invoice_details',$save_details);
	...
	if($TranPageName=='SaleEntry')
	{
		...
		if($datatype=='SAVE')
		{
									...
									//HEADER SECTION		
									//$save_hdr['comment']=mysql_real_escape_string($form_data->comment);
									$RELATED_TO_MIXER=mysql_real_escape_string($form_data->RELATED_TO_MIXER);
									$save_hdr['invoice_date']=mysql_real_escape_string($form_data->invoice_date);
									$save_hdr['challan_no']=mysql_real_escape_string($form_data->challan_no);
									$save_hdr['challan_date']=mysql_real_escape_string($form_data->challan_date);
									$save_hdr['comment']=mysql_real_escape_string($form_data->comment);
									...
									$save_hdr['BILL_TYPE']=mysql_real_escape_string($form_data->BILL_TYPE);

									$doctor_ledger_id=$save_hdr['doctor_ledger_id']=mysql_real_escape_string($form_data->doctor_ledger_id);
									...
									else
									{
										...
										$invsrl=1;
										$sql="select max(srl) srl  from invoice_summary 	where  finyr='$finyr' and status='SALE' and company_id=".$company_id;				
									...
									if($RELATED_TO_MIXER=='YES')//FOR MIXER PRODUCT ENTRY
									{
													...
													$save_details_mix['RELATED_TO_MIXER']=mysql_real_escape_string($form_data->RELATED_TO_MIXER);
													$save_details_mix['product_name']='MIXTURE';		
													$save_details_mix['qnty']=mysql_real_escape_string($form_data->qnty_mixture);
													
													if(mysql_real_escape_string($form_data->MIX_RAW_LINK_ID)>0)
													{
														$MIX_RAW_LINK_ID=mysql_real_escape_string($form_data->MIX_RAW_LINK_ID);
												...
												//	$save_details['purchase_rate']=mysql_real_escape_string($form_data->purchase_rate);
													...
													$save_details['Synonym']=mysql_real_escape_string($form_data->Synonym);
													$save_details['label_print']=mysql_real_escape_string($form_data->label_print);
	...
	private function check_own_company_and_transfer($id_header=0)	
	{
	...
	{	
			...
			$data['param5'] =$param5;
			$data['user_accounts'] ='';
			$data['ledger_accounts'] =$data['user_ac']=	$data['ledger_ac']='';
			...
			if( $REPORT_NAME=='HSN_WISE_SALE' || $REPORT_NAME=='HSN_WISE_SUMMARY' ||$REPORT_NAME=='GST_REPORT' || $REPORT_NAME=='DOCTOR_COMMISSION_SUMMARY' || 
