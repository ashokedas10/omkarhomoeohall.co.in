...
{ 
	...
	$token_shift=$patient_prescription[0]->token_shift;
	
	if($token_shift=='MORNING')
	{$token_shift='MOR';}
	else
	{$token_shift='EVE';}
	...
	<div  class="page"  >
		...
		<table  style="width:100%" border="0" cellpadding="0" cellspacing="0" >		
				...
				<td align="center" class="doctorname" colspan="2"><?PHP echo $doc_name; ?></td>
				<td width="21%" align="left" class="patient_id">Token No :<?PHP echo $token_no.' ('.$token_shift.')'; ?> </td>	
				</tr>		
		...
		and doctor_mstr_id=".$doctor_mstr_id." ORDER BY ID DESC";
		$prescriptions = $this->projectmodel->get_records_from_sql($prescriptions);			
		foreach ($prescriptions as $prescription)
		{
			/*$patient_prescription="select *	from patient_prescription where id=".$prescription_id;	
			$patient_prescription = $this->projectmodel->get_records_from_sql($patient_prescription);	
			$prescription_date=$patient_prescription[0]->prescription_date;
			$ACTUAL_VISIT_AMT=$patient_prescription[0]->ACTUAL_VISIT_AMT;	
			$prescription_id=$patient_prescription[0]->id;*/
		
		$product_print=$product_print.'<br>'.'Dt:'.$prescription->prescription_date.
		' Visit:'.$prescription->ACTUAL_VISIT_AMT;
		
		}
		$product_print=$product_print.'<br><br>';
		...
		//$ACTUAL_VISIT_AMT=$prescription_date='';
		//$PRES_N0=1;	
		$records = "select *
		from invoice_summary where patient_id=".$patient_registration_id." 
		...
		{ 
			...
			{
				$product_print=$product_print.'Bill:"'.$record->invoice_no.' Dt:'.$record->invoice_date;
				$products=array();
				$products=$this->projectmodel->bill_wise_item_formating($invoice_summary_id);
				foreach ($products as $product)
				{
					foreach ($product as $prd)
					{$product_print=$product_print.'<br>'.$prd;}
				}
