...
public function create_report($rs=array(),$id=0)
{
    ...
    foreach($resval['header'][0]['fields'] as $key1=>$flds)
    {
            foreach($flds as $key2=>$fld)
            {
                ...
                $save_details[$fld['LabelName']]=$fld['Inputvalue'];
            }							
                ...
                {	
                    ...
                    foreach ($header as $key2=>$field_val)
                    {
                        if($sections['section_type']=='GRID_ENTRY')
                        {
                            ...
                            $form_structure['header'][$key]['fields'][0][$key2]=
                            $this->create_fields_parameter($sections['frmrpttemplatehdr_id'],$key2,0,$input_id_index);
                            ...
                            $form_structure['header'][$key]['TableName']=$sections['TableName'];
                            //here
                            $form_structure['header'][$key]['fields'][$key1+1][$key2]=
    ...
    {
        $output=array();
        $details="select * from  frmrpttemplatedetails  
        where frmrpttemplatehdrID=".$frmrpttemplatedetails_id." and InputName='$InputName' order by FieldOrder";					
        ...
        {	
            ...
            // else {$output['InputType']='text';}
                ...
                if($detail->datafields<>'')
                {
                    ...
                    $output['datafields']=json_decode(json_encode($output['datafields']), true);
                }
                else
                {$output['datafields']='';}				
                $inputval='';
			...
			{				
					...
					{	
                        ...
                        {$form_structure['header'][$indx]['body'][$key2]['datafields']=
                            $this->projectmodel->get_records_from_sql($detail->datafields);}
						else
						{$form_structure['header'][$indx]['body'][$key2]['datafields']='';}				
					}	
    ...
    public function create_report_for_web($rs=array(),$id=0)
    {
        $return_data=$output=array();
        $resval=$this->create_form($rs,$id); 
